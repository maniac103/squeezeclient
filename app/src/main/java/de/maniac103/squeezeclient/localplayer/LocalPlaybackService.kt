...
package de.maniac103.squeezeclient.localplayer
...
class LocalPlaybackService : Service(), LifecycleOwner, LocalPlayer.Listener {
    ...
    override fun onHeadersReceived(response: Response) {
        lifecycleScope.launch {
            slimproto.sendResponseReceived(response)
        }
    }

    override fun onMetadataReceived(title: CharSequence, artworkUri: Uri?) {
        lifecycleScope.launch {
    ...
    private fun updateForegroundNotification(state: SlimprotoState) {
        ...
        val content = when (state) {
            ...
            is SlimprotoState.Stopped ->
                getString(R.string.local_player_notification_content_stopped)
            is SlimprotoState.Disconnected ->
                getString(R.string.local_player_notification_content_disconnected)
        }
        val notification = NotificationCompat.Builder(this, channelId)
            .setContentTitle(getString(R.string.local_player_notification_title))
    ...
    private suspend fun handleCommand(command: SlimprotoSocket.CommandPacket) {
        when (command) {
            ...
            is SlimprotoSocket.CommandPacket.StreamStart -> {
                ...
                player.start(command.uri, command.headers, command.autoStart)
            }
