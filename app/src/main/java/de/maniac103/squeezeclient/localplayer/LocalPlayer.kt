...
class LocalPlayer(context: Context, private val listener: Listener) : Player.Listener {
    ...
    interface Listener {
        ...
        fun onPlaybackEnded()
        fun onHeadersReceived(response: Response)
        fun onMetadataReceived(title: CharSequence, artworkUri: Uri?)
    }
    ...
    init {
        val client = context.httpClient.newBuilder()
            .addInterceptor { chain ->
                ...
                listener.onHeadersReceived(response)
                response
            }
            .build()
    ...
    @OptIn(UnstableApi::class)
    fun start(uri: Uri, headers: Map<String, String>, autoStart: Boolean) {
    ...
    override fun onPlayWhenReadyChanged(playWhenReady: Boolean, reason: Int) {
        ...
        if (readyForPlayback) {
            listener.onPauseStateChanged(!playWhenReady)
        }
        updatePlayerVolume(false)
    }
    ...
    override fun onMediaMetadataChanged(mediaMetadata: MediaMetadata) {
        ...
        mediaMetadata.title?.let { title ->
            listener.onMetadataReceived(title, mediaMetadata.artworkUri)
        }
    }
